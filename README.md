# mutt-wizard for termux on android
...
- run pass account_name before syncing because termux can't promt you any gui decrypt window.

The rest is original readme-

Get this great stuff without effort:

- A full-featured and autoconfigured email client on the terminal with mutt
...
- Creates dotfiles for `mutt`, `isync`, and `msmtp` appropriate for your email address
...
- Provides sensible defaults and an attractive appearance for the mutt email client
...
The mutt-wizard is run with the command `mw`. Once everything is setup, you'll use `mutt` to access your mail.
...
There's a chance of errors if you use a slow-release distro like Ubuntu, Debian or Mint. If you get errors in `mutt`, install the most recent version manually or manually remove the offending lines in the config in `/usr/share/mutt-wizard/mutt-wizard.muttrc`.
...
- More elegant attachment handling. Image/video/pdf attachments without relying on the mutt instance.
...
- The critical `mutt`/`mutt` files are in `~/.config/mutt/`.
